...
const RELATED_SERVICES = [
  ...
  { href: "/las-vegas/slab-leak-detection-repair/", text: "Las Vegas slab leak detection and repair" },
];
              ...
              <ul className="mt-4 flex flex-wrap gap-3">
                {RELATED_SERVICES.map((r) => (
                  <li key={r.href}>
                    <Link
                      href={r.href}
                      className="inline-flex rounded-lg bg-white px-4 py-2 text-sm font-semibold text-brand-dark shadow-sm ring-1 ring-brand-surface-alt hover:text-brand-dark/70"
                    >
                      {r.text}
                    </Link>
                  </li>
